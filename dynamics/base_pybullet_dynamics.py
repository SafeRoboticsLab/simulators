...
class BasePybulletDynamics(BaseDynamics):
  ...
  def __init__(self, cfg: Any, action_space: np.ndarray) -> None:
    ...
    """
        ...
        Args:
            cfg (Any): an object specifies configuration. This will correspond to cfg_agent of the yaml config file
            action_space (np.ndarray): action space.
        """

    self.verbose = cfg.VERBOSE
    self.gui = cfg.GUI
    self.gui_imaginary = cfg.GUI_IMAGINARY
    self.dt = cfg.dt
    self.gravity = -9.81
    ...
    self.force_applied_reset = cfg.FORCE_RESET_TIME

    self.rotate_reset = cfg.ROTATE_RESET
    self.height_reset = cfg.HEIGHT_RESET

    self.force_applied_force_vector = None
    self.force_applied_position_vector = None
    self.force_random = cfg.FORCE_RANDOM

    # configure terrain in the dynamics
    self.terrain = cfg.TERRAIN
    self.terrain_height = cfg.TERRAIN_HEIGHT
    self.terrain_friction = cfg.TERRAIN_FRICTION
    ...
    if self.gui:
      ...
      p.resetDebugVisualizerCamera(
          cameraDistance=1, cameraYaw=20, cameraPitch=-20,
  ...
  def _init_frames(self):
    ...
    if self.video_output_file is not None:
      ...
      command = [
          'ffmpeg', '-y', '-r',
          str(24), '-f', 'rawvideo', '-vcodec', 'rawvideo', '-s',
          '{}x{}'.format(camera[0], camera[1]), '-pix_fmt', 'rgba', '-i', '-',
          '-an', '-vcodec', 'mpeg4', '-vb', '20M', self.video_output_file
      ]
      #print(command)
  ...
  def _save_frames(self):
    ...
    if self.video_output_file is not None and \
      ...
      img = p.getCameraImage(
          camera[0], camera[1], renderer=p.ER_BULLET_HARDWARE_OPENGL,
  ...
  def reset(self, **kwargs):
    ...
    p.setPhysicsEngineParameter(
        fixedTimeStep=self.dt, physicsClientId=self.client
    )
  ...
  def integrate_forward(
    ...
    return super().integrate_forward(
        state, control, num_segment, noise, noise_type, adversary, **kwargs
    )
  ...
  def _apply_adversarial_force(self, force_vector, position_vector):
    ...
    p.applyExternalForce(
        self.robot.id, -1, self.force_applied_force_vector,
        self.force_applied_position_vector, p.LINK_FRAME,
        physicsClientId=self.client
    )
  ...
  def _apply_force(self):
    ...
    p.addUserDebugLine(
        ...
        self.force_applied_position_vector + self.force_applied_force_vector,
        lineColorRGB=[0, 0, 1], lineWidth=2.0, lifeTime=0.1,
        physicsClientId=self.client, parentObjectUniqueId=self.robot.id
    )

  def _gen_terrain(
      self, terrain_height: Optional[int] = None,
    ...
    p.changeDynamics(
        terrain, -1, lateralFriction=self.terrain_friction,
        physicsClientId=self.client
    )
    p.changeVisualShape(
        terrain, -1, rgbaColor=[0.2, 0.8, 0.8, 1], physicsClientId=self.client
    )

    self.terrain_data = heightfieldData

  def _set_terrain(
      self, terrain_data, terrain_height: Optional[int] = None,
      mesh_scale: Optional[np.ndarray] = None
  ):
    """
        Create a randomized terrain to be applied into the dynamics
    ...
    terrainShape = p.createCollisionShape(
        shapeType=p.GEOM_HEIGHTFIELD, meshScale=mesh_scale,
        heightfieldTextureScaling=(numHeightfieldRows-1) / 2,
