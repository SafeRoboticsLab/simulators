...
class Spirit:
  ...
  def __init__(
      self, client, height, orientation, envtype=None, payload=0,
      payload_max=0, **kwargs
  ):
    self.client = client
    ...
    self.id = p.loadURDF(
        fileName=f_name, basePosition=[ox, oy, self.height],
        baseOrientation=orientation, physicsClientId=client
    )
  ...
  def reset(self, position):
    for i in range(len(self.joint_index)):
      p.resetJointState(
          self.id, self.joint_index[i], position[i],
  ...
  def apply_action(self, action):
    ...
    for i in range(len(self.joint_index)):
      info = p.getJointInfo(
          self.id, self.joint_index[i], physicsClientId=self.client
      )
      lower_limit = info[8]
      upper_limit = info[9]
      max_force = info[10]
      ...
      p.setJointMotorControl2(
          ...
          positionGain=1. / 12., velocityGain=0.4, force=max_force,
          maxVelocity=max_velocity, physicsClientId=self.client
      )
  ...
  def get_obs(self):
    ...
    pos, ang = p.getBasePositionAndOrientation(
  ...
  def get_joint_position(self):
    joint_state = p.getJointStates(
        self.id, jointIndices=self.joint_index, physicsClientId=self.client
    )
  ...
  def make_joint_list(self):
    damaged_legs = []
    joint_names = [
        b'hip0', b'upper0', b'lower0', b'hip1', b'upper1', b'lower1', b'hip2',
        b'upper2', b'lower2', b'hip3', b'upper3', b'lower3'
    ]
    joint_list = []
  ...
  def linc_get_joints_positions(self):
    ...
    for joint in self.joint_index:
      if joint != 1000:
        pos[i] = p.getJointState(self.id, joint,
  ...
  def get_elbows(self):
    ...
    FL_elbow = rotate_x(roll) @ rotate_y(pitch) @ rotate_z(yaw) @ (
        translate_x(hip_from_body_l) + rotate_x(-np.pi * 0.5) @ (
            translate_z(hip_from_body_w) + rotate_x(hip_FL) @ (
                ...
                + rotate_z(np.pi - upper_FL) @ (
                    translate_x(upper_link_l) + rotate_z(-np.pi + upper_FL)
                    @ rotate_x(-hip_FL) @ rotate_x(np.pi * 0.5) @ initial_pos
                )
            )
        )
    )

    FR_elbow = rotate_x(roll) @ rotate_y(pitch) @ rotate_z(yaw) @ (
        translate_x(hip_from_body_l) + rotate_x(np.pi * 0.5) @ (
            translate_z(hip_from_body_w) + rotate_x(-hip_FR) @ (
                translate_x(hip_l * 0.5) + translate_z(upper_from_hip_w)
                + rotate_z(np.pi + upper_FR) @ (
                    translate_x(upper_link_l) + rotate_z(-np.pi - upper_FR)
                    @ rotate_x(hip_FR) @ rotate_x(-np.pi * 0.5) @ initial_pos
                )
            )
        )
    )

    BL_elbow = rotate_x(roll) @ rotate_y(pitch) @ rotate_z(yaw) @ (
        translate_x(-hip_from_body_l) + rotate_x(-np.pi * 0.5) @ (
            translate_z(hip_from_body_w) + rotate_x(hip_BL) @ (
                translate_x(hip_l * 0.5) + translate_z(upper_from_hip_w)
                + rotate_z(np.pi - upper_BL) @ (
                    translate_x(upper_link_l) + rotate_z(-np.pi + upper_BL)
                    @ rotate_x(-hip_BL) @ rotate_x(np.pi * 0.5) @ initial_pos
                )
            )
        )
    )

    BR_elbow = rotate_x(roll) @ rotate_y(pitch) @ rotate_z(yaw) @ (
        translate_x(-hip_from_body_l) + rotate_x(np.pi * 0.5) @ (
            translate_z(hip_from_body_w) + rotate_x(-hip_BR) @ (
                translate_x(hip_l * 0.5) + translate_z(upper_from_hip_w)
                + rotate_z(np.pi + upper_BR) @ (
                    translate_x(upper_link_l) + rotate_z(-np.pi - upper_BR)
                    @ rotate_x(hip_BR) @ rotate_x(-np.pi * 0.5) @ initial_pos
                )
            )
        )
    )
  ...
  def get_body_corners(self):
    ...
    FL_top = rotate_x(roll) @ rotate_y(pitch) @ rotate_z(yaw) @ (
        translate_x(L * 0.5) + rotate_x(-np.pi * 0.5) @ (
            translate_z(W * 0.5)
            + rotate_x(np.pi * 0.5) @ (translate_z(H * 0.5) + initial_pos)
        )
    )
    FL_bot = rotate_x(roll) @ rotate_y(pitch) @ rotate_z(yaw) @ (
        translate_x(L * 0.5) + rotate_x(-np.pi * 0.5) @ (
            translate_z(W * 0.5)
            + rotate_x(np.pi * 0.5) @ (translate_z(-H * 0.5) + initial_pos)
        )
    ...
    FR_top = rotate_x(roll) @ rotate_y(pitch) @ rotate_z(yaw) @ (
        translate_x(L * 0.5) + rotate_x(np.pi * 0.5) @ (
            translate_z(W * 0.5)
            + rotate_x(-np.pi * 0.5) @ (translate_z(H * 0.5) + initial_pos)
        )
    )
    FR_bot = rotate_x(roll) @ rotate_y(pitch) @ rotate_z(yaw) @ (
        translate_x(L * 0.5) + rotate_x(np.pi * 0.5) @ (
            translate_z(W * 0.5)
            + rotate_x(-np.pi * 0.5) @ (translate_z(-H * 0.5) + initial_pos)
        )
    ...
    BL_top = rotate_x(roll) @ rotate_y(pitch) @ rotate_z(yaw) @ (
        translate_x(-L * 0.5) + rotate_x(-np.pi * 0.5) @ (
            translate_z(W * 0.5)
            + rotate_x(np.pi * 0.5) @ (translate_z(H * 0.5) + initial_pos)
        )
    )
    BL_bot = rotate_x(roll) @ rotate_y(pitch) @ rotate_z(yaw) @ (
        translate_x(-L * 0.5) + rotate_x(-np.pi * 0.5) @ (
            translate_z(W * 0.5)
            + rotate_x(np.pi * 0.5) @ (translate_z(-H * 0.5) + initial_pos)
        )
    ...
    BR_top = rotate_x(roll) @ rotate_y(pitch) @ rotate_z(yaw) @ (
        translate_x(-L * 0.5) + rotate_x(np.pi * 0.5) @ (
            translate_z(W * 0.5)
            + rotate_x(-np.pi * 0.5) @ (translate_z(H * 0.5) + initial_pos)
        )
    )
    BR_bot = rotate_x(roll) @ rotate_y(pitch) @ rotate_z(yaw) @ (
        translate_x(-L * 0.5) + rotate_x(np.pi * 0.5) @ (
            translate_z(W * 0.5)
            + rotate_x(-np.pi * 0.5) @ (translate_z(-H * 0.5) + initial_pos)
        )
    ...
    return np.concatenate([
        FL_top, FL_bot, FR_top, FR_bot, BL_top, BL_bot, BR_top, BR_bot
    ], axis=1)
