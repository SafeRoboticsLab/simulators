...
class LinearPolicy(BasePolicy):
  ...
  def __init__(
      self, id: str, nominal_states: np.ndarray, nominal_controls: np.ndarray, K_closed_loop: np.ndarray,
      k_open_loop: np.ndarray, cfg: Any, **kwargs
  ):
    super().__init__(id, cfg)
    self.policy_type = "Linear"

    # Params
    assert nominal_states.shape[-1] == nominal_controls.shape[-1] == K_closed_loop.shape[-1] == k_open_loop.shape[-1]
  ...
  def get_action(self, obsrv: np.ndarray, agents_action: Optional[Dict[str, np.ndarray]] = None,
    ...
    time0 = time.time()
    action = (
        self.nominal_controls[:, time_idx]
        + self.K_closed_loop[..., time_idx] @ (state - self.nominal_states[:, time_idx])
